...
public:
    Weapon(int wKindNo_, Warrior * wpwarrior_) : wKindNo(wKindNo_), wpwarrior(wpwarrior_) {}
...
public:
    ...
    bool useful() { return (swordPower != 0); }                // 是否还有攻击力
...
public:
    ...
    Arrow(int wKindNo_, Warrior * wpwarrior_) : Weapon(wKindNo_, wpwarrior_) { arrowuse = 3; }
...
public:
    bool bombuse;
    Bomb(int wKindNo_, Warrior * wpwarrior_) : Weapon(wKindNo_, wpwarrior_) { bombuse = false; }
};
...
public:
    ...
    //    virtual void Attack(Warrior * uApwarrior);
    //    virtual void Hurted(int nPower);
    //    virtual void FightBack(Warrior * uFpwarrior);
};
...
private:
    ...
    int FightResult[2];             // 记录战斗结果; (-1, -1)没发生战斗, (0, -1): 红胜一场;
    // (0, 0)连续两场红胜; (0, 1) 蓝胜一场
public:
    City(int cityLifeValue_, int cityNum_, int flag_ = -1) : cityNum(cityNum_), cityLifeValue(cityLifeValue_), flag(flag_) { FightResult[0] = -1; FightResult[1] = -1; }
    void Init(Warrior * pRedWarrior, Warrior * pBlueWarrior);
};
...
Warrior::Warrior(Headquarter * p, int no_, int kindNo_)
{
    ...
    for (int i = 0; i < weaponCount; i++)
    {
        ...
        else if (weapon_num == 1) weapon[i] = new Arrow(weapon_num, this);
        else if (weapon_num == 2) weapon[i] = new Bomb(weapon_num, this);
    }
}
...
void Dragon::PrintResult(int nTime)
{
    ...
    cout << "It has a " << weapon[0]->getweaponName() << ",";
...
{
    Warrior::PrintResult(nTime);
    cout << "It has a " << weapon[0]->getweaponName() << " and a " << weapon[1]->getweaponName() << endl;
}

void Iceman::PrintResult(int nTime)
{
    Warrior::PrintResult(nTime);
    cout << "It has a " << weapon[0]->getweaponName() << endl;
}
